package reflex.graphics.attributes
{
	...
	public class Color
	{
		...
		private static var regex1:RegExp = /^#([a-zA-Z0-9]{6}|[a-zA-Z0-9]{3})$/;
		private static var regex2:RegExp = /^rgb\((\d{1,3})(%?),(\d{1,3})\2,(\d{1,3})\2\)$/;
		
		/**
		 * Converts a string representation of a color into an int that can be
		 * use in fills, strokes, etc.
		 * 
		 * @param The color string.
		 * @return A valid color uint
		 */
		public static function fromString(color:String):uint
		{
			...
			} else if ( (matches = color.match(regex2)) ) {
